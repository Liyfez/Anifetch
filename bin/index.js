  ...
  try {
    while (true) {
      if (step === 1) {
        ...
        const { text, escaped } = await ask(`${c.hotPink("Export format")} ${c.dim("[json / csv / txt / md / all] (default: json) [ESC: back]:")} `);
        if (escaped) {
        ...
        const { text, escaped } = await ask(`${c.hotPink("Filter status")} ${c.dim("[all / completed / watching / dropped / planning] (default: all) [ESC: back]:")} `);
        if (escaped) {
